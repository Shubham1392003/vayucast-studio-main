...
const AirQualityMap = () => {
  ...
  const fetchAQIData = async (lat: number, lon: number) => {
    try {
      ...
      // 3. Try fetching from the new Django ML Backend
      let backendPredicted = null;
      try {
        const mlRes = await fetch("http://localhost:8000/api/predict/", {
          method: "POST",
        ...
        if (mlRes.ok) {
           ...
           if (mlData.predicted_pm25) {
             backendPredicted = Math.round(mlData.predicted_pm25);
           }
        }
      } catch (e) {
        console.warn("Django ML Backend unavailable or failed, falling back to simulated prediction", e);
      }
  ...
  const { predictedAQI: simPredicted, confidence: simConfidence } = simulateRandomForestPrediction(
    data.historicalAqi,
    data.windSpeed,
    data.temperature
  );

  const predictedAQI = data.backendPredicted ?? simPredicted;
  const confidence = data.backendPredicted ? 98 : simConfidence;

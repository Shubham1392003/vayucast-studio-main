...
const DEFAULT_LOCATION = { lat: 18.5204, lon: 73.8567 }; // Pune
...
const PollutionSpread = () => {
  ...
  const [data, setData] = useState({ aqi: 0, windSpeed: 0, windDirection: 0 });
  ...
  // Generate mock areas around the location
  const generateAreas = (lat: number, lng: number, baseAqi: number) => {
  ...
  const fetchData = useCallback(async (lat: number, lng: number) => {
    try {
      ...
      const [aqiRes, weatherRes] = await Promise.all([
        ...
        fetch(`https://api.open-meteo.com/v1/forecast?latitude=${lat}&longitude=${lng}&current=wind_speed_10m,wind_direction_10m`)
      ]);

      const aqiJson = await aqiRes.json();
      const weatherJson = await weatherRes.json();

      const baseAqi = aqiJson?.current?.us_aqi || 50;
      const wSpeed = weatherJson?.current?.wind_speed_10m || 10;
      const wDir = weatherJson?.current?.wind_direction_10m || 90;
      ...
      generateAreas(lat, lng, baseAqi);
    } catch (err) {
      console.error(err);
      // Fallback
      setData({ aqi: 120, windSpeed: 15, windDirection: 135 });
      generateAreas(lat, lng, 120);
    } finally {
      setLoading(false);
    }
  }, []);
    ...
    if (navigator.geolocation) {
      ...
      navigator.geolocation.getCurrentPosition(
        ...
        () => {
          setLoading(false);
          if (!location) {
            setLocation({ lat: DEFAULT_LOCATION.lat, lng: DEFAULT_LOCATION.lon });
  ...
  const spreadCenter = useMemo(() => {
    ...
    const latOffset = -Math.cos((data.windDirection * Math.PI) / 180) * 0.0003; 
    const lngOffset = -Math.sin((data.windDirection * Math.PI) / 180) * 0.0003;

    return {
      lat: location.lat + (latOffset * simulationStep),
      lng: location.lng + (lngOffset * simulationStep),
    };
  }, [location, data.windDirection, simulationStep]);
  ...
  const windLines = useMemo(() => {
    ...
    const lines = [];
    const latOffset = -Math.cos((data.windDirection * Math.PI) / 180) * 0.03; // length of wind visual line
    const lngOffset = -Math.sin((data.windDirection * Math.PI) / 180) * 0.03;
  ...
  const affectedAreas = useMemo(() => {
    ...
    return generatedAreas.map(j => {
      ...
      if (dist < spreadRadius) {
        ...
        const intensity = Math.max(0, Math.floor(data.aqi * (1 - dist/spreadRadius) * 0.4));
        return { ...j, addedAqi: intensity };
      }
      return null;
    }).filter(Boolean) as (Area & {addedAqi: number})[];
  }, [spreadCenter, spreadRadius, generatedAreas, data.aqi]);
  ...
  const handleRun = () => {
    ...
    const interval = setInterval(() => {
      step += 1;
      setSpreadRadius(prev => prev + 50 + (data.windSpeed * 0.5)); // Radius expands faster with high wind speed
  ...
  const spreadColor = getAqiHexColor(data.aqi);
          ...
          <div className="ml-auto flex flex-wrap items-center gap-3 justify-end sm:gap-6">
            <div className="flex items-center gap-2 bg-secondary/50 px-3 py-1.5 rounded-lg">
              ...
              <div>
                ...
                <p className="text-xs font-bold text-foreground">{loading ? "..." : getWindDirectionStr(data.windDirection)} @ {loading ? "..." : data.windSpeed} km/h</p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-2 bg-secondary/50 px-3 py-1.5 rounded-lg">
              ...
              <div>
                ...
                <p className="text-xs font-bold text-foreground">{loading ? "..." : data.aqi}</p>
              </div>
            </div>

            <Button className="rounded-full px-6 shadow-glow" onClick={handleRun} disabled={isRunning || loading || !location}>
        ...
        <div className="relative flex-1 min-h-[400px] bg-secondary/20">
          {loadError ? (
            ...
            >
                ...
                <Polyline
                  ...
                  options={{
                    ...
                    icons: [{
                      ...
                      offset: `${windOffset}%`, // Animate offset across the path
                    }]
                  }}
                />
              ))}
              ...
              {/* Spread Polygon/Circle: High Effect Core (Inner 40%) */}
              ...
              {/* Spread Polygon/Circle: Low Effect Zone (Outer 100%) */}
              {spreadRadius > 0 && spreadCenter && (
                <Circle
                  center={spreadCenter}
                  radius={spreadRadius}
              ...
              {selectedMarker === "source" && (
                <InfoWindow position={location} onCloseClick={() => setSelectedMarker(null)}>
                  <div className="p-1 min-w-[120px]">
                    ...
                    <p className="text-xs text-muted-foreground mt-1">AQI: <span className="font-bold text-foreground">{data.aqi}</span></p>
                    <p className="text-[10px] text-muted-foreground">Click map to move pin.</p>
                  </div>
                </InfoWindow>
              )}
              ...
              {generatedAreas.map(j => selectedMarker === j.id && (
                <InfoWindow key={`info-${j.id}`} position={{ lat: j.lat, lng: j.lng }} onCloseClick={() => setSelectedMarker(null)}>
                  <div className="p-1 min-w-[140px]">
                    ...
                    <p className="text-xs text-muted-foreground mt-1">Base AQI: <span className="font-bold text-foreground">{j.baseAqi}</span> ({getAqiCategory(j.baseAqi)})</p>
        ...
        {affectedAreas.length > 0 ? (
          <Card className="border-destructive/30 bg-destructive/10 p-4 shadow-card mt-4 backdrop-blur-sm">
            <div className="flex gap-3">
              ...
              <div>
                <p className="text-xs font-bold text-destructive mb-1">Evacuation Warning</p>
                <p className="text-[11px] text-foreground/90 leading-relaxed">
                  Hazardous plume is tracking towards densely populated sectors. Issue immediate shelter-in-place alerts.
                </p>
              </div>
            </div>
          </Card>
